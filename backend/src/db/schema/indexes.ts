...
export const cruiseActiveIndex = index('cruises_active_idx').on(
  cruises.showCruise,
  cruises.isActive
);
export const cruiseEmbarkPortIndex = index('cruises_embark_port_idx').on(cruises.embarkPortId);
export const cruiseDisembarkPortIndex = index('cruises_disembark_port_idx').on(
  cruises.disembarkPortId
);
...
export const altSailingsBaseCruiseIndex = index('alt_sailings_base_cruise_idx').on(
  alternativeSailings.baseCruiseId
);
export const altSailingsAltCruiseIndex = index('alt_sailings_alt_cruise_idx').on(
  alternativeSailings.alternativeCruiseId
);
export const altSailingsDateIndex = index('alt_sailings_date_idx').on(
  alternativeSailings.sailingDate
);
...
export const cabinCategoryTypeIndex = index('cabin_categories_category_idx').on(
  cabinCategories.category
);
export const cabinCategoryActiveIndex = index('cabin_categories_active_idx').on(
  cabinCategories.isActive
);
...
export const pricingRateCodeIndex = index('pricing_rate_code_idx').on(pricing.rateCode);
...
export const cheapestPricingCruiseIndex = uniqueIndex('cheapest_pricing_cruise_idx').on(
  cheapestPricing.cruiseId
);
export const cheapestPricingPriceIndex = index('cheapest_pricing_price_idx').on(
  cheapestPricing.cheapestPrice
);
export const cheapestPricingInteriorIndex = index('cheapest_pricing_interior_idx').on(
  cheapestPricing.interiorPrice
);
export const cheapestPricingOceanviewIndex = index('cheapest_pricing_oceanview_idx').on(
  cheapestPricing.oceanviewPrice
);
export const cheapestPricingBalconyIndex = index('cheapest_pricing_balcony_idx').on(
  cheapestPricing.balconyPrice
);
export const cheapestPricingSuiteIndex = index('cheapest_pricing_suite_idx').on(
  cheapestPricing.suitePrice
);

// Quote request indexes
// Quote request indexes - updated for production schema
export const quoteRequestCruiseIndex = index('quote_requests_cruise_idx').on(
  quoteRequests.cruiseId
);
export const quoteRequestStatusIndex = index('quote_requests_status_idx').on(quoteRequests.status);
export const quoteRequestCreatedIndex = index('quote_requests_created_idx').on(
  quoteRequests.createdAt
);
// Removed indexes for fields that don't exist in production: userId, quoteExpiresAt
...
export const savedSearchLastCheckedIndex = index('saved_searches_last_checked_idx').on(
  savedSearches.lastChecked
);
...
export const allIndexQueries = [...fullTextSearchQueries, ...jsonbIndexQueries];
