 ...
 */
export function generateSlugVariations(shipName: string, departureDate: string, cruiseId: number): string[] {
  const variations: string[] = [];
  
  // Original slug
  const originalSlug = generateCruiseSlug({ shipName, departureDate, cruiseId });
...
/**
 * Extract search-friendly terms from a slug for better matching
 */
export function extractSearchTermsFromSlug(slug: string): string[] {
  ...
  // Add ship name parts
  const shipParts = parsed.shipName.split('-').filter(part => part.length > 2);
  terms.push(...shipParts);
  
  // Add date components
  terms.push(parsed.departureDate);
  terms.push(parsed.cruiseId.toString());
  
  return terms;
}

/**
 * Create a slug from cruise database record
 */
export function createSlugFromCruiseData(cruise: {
  id: number;
  shipName?: string;
  sailingDate: string;
}): string {
...
/**
 * Validate and normalize a slug for database queries
 */
export function normalizeSlugForQuery(slug: string): string {
  const parsed = parseCruiseSlug(slug);
  if (!parsed) {
    throw new Error('Invalid cruise slug format');
  }
