...
export class BulkFtpDownloaderService {
  ...
  async downloadLineUpdates(
    ...
    if (cruiseInfos.length > this.MEGA_BATCH_SIZE) {
      logger.warn(
        `⚠️ Cruise list too large (${cruiseInfos.length}). Processing first ${this.MEGA_BATCH_SIZE} cruises to prevent FTP overload`,
        {
          lineId,
    ...
    if (this.circuitBreakerState.isOpen) {
      ...
      if (timeSinceFailure < this.CIRCUIT_RESET_TIMEOUT) {
        throw new Error(
          `Circuit breaker is OPEN for FTP downloads. Retry in ${Math.round((this.CIRCUIT_RESET_TIMEOUT - timeSinceFailure) / 1000)}s`
        );
      } else {
    ...
    try {
      ...
      logger.info(`🚀 [BULK-DOWNLOAD] Starting bulk download for line ${lineId}`, {
        ...
        avgFilesPerShip: Math.round(
          cruiseInfos.length / Array.from(new Set(cruiseInfos.map(c => c.shipName))).length
        ),
      ...
      for (let i = 0; i < shipNames.length; i += shipsPerChunk) {
        ...
        const chunkCruiseCount = shipChunk.reduce(
          (sum, ship) => sum + cruisesByShip.get(ship)!.length,
          0
        );
  ...
  private async downloadShipFiles(
    ...
    logger.info(
      `📁 BULK FTP DEBUG: Using webhook line ID ${webhookLineId} for FTP paths (database line ID: ${lineId})`,
      {
        lineId,
        webhookLineId,
        shipKey,
    ...
    for (let i = 0; i < cruises.length; i++) {
      ...
      if (i % 10 === 0 || i === cruises.length - 1 || i === 0) {
        logger.info(
          `📥 [FILE-DOWNLOAD] Downloading cruise file ${i + 1}/${cruises.length} for ship ${shipKey}`,
          {
            cruiseId: cruise.id,
            shipKey,
            lineId,
    ...
    logger.info(`✅ BULK FTP DEBUG: Completed ship ${shipKey}`, {
      ...
      shipFailed,
      successRate:
        shipSuccessful > 0 ? `${Math.round((shipSuccessful / cruises.length) * 100)}%` : '0%',
  ...
  private async downloadSingleCruiseFile(
    ...
    if (cruise.shipId) {
      ...
      possiblePaths.push(
        `/isell_json/${sailingYear}/${sailingMonth}/${webhookLineId}/${cruise.shipId}/${fileName}`
      );
    }
    ...
    possiblePaths.push(
      `/isell_json/${sailingYear}/${sailingMonth}/${webhookLineId}/${processedShipName}/${fileName}`
    );
    ...
    if (cruise.cruiseCode && cruise.cruiseCode !== cruise.id) {
      ...
      if (cruise.shipId) {
        ...
        possiblePaths.push(
          `/isell_json/${sailingYear}/${sailingMonth}/${webhookLineId}/${cruise.shipId}/${cruiseCodeFileName}`
        );
      }
      possiblePaths.push(
        `/${sailingYear}/${sailingMonth}/${webhookLineId}/${processedShipName}/${cruiseCodeFileName}`
      );
      possiblePaths.push(
        `/isell_json/${sailingYear}/${sailingMonth}/${webhookLineId}/${processedShipName}/${cruiseCodeFileName}`
      );
    }
    ...
    try {
      ...
      if (!data) {
        throw new Error(
          `All ${uniquePaths.length} path variations failed. Paths tried: ${uniquePaths.slice(0, 3).join(', ')}${uniquePaths.length > 3 ? '...' : ''}`
        );
      }
      ...
      try {
        ...
        throw new Error(
          `JSON parse failed: ${parseError instanceof Error ? parseError.message : 'Invalid JSON'}`
        );
      }
      ...
      if (
        ...
        errorMsg.includes('path variations failed')
      ) {
  ...
  private async getConnection(): Promise<ftp.Client> {
    ...
    try {
      ...
      const credentialStatus = {
        ...
        hostPreview: env.TRAVELTEK_FTP_HOST
          ? env.TRAVELTEK_FTP_HOST.substring(0, 10) + '***'
          : 'MISSING',
        userStatus: env.TRAVELTEK_FTP_USER ? 'LOADED' : 'MISSING',
        userPreview: env.TRAVELTEK_FTP_USER
          ? env.TRAVELTEK_FTP_USER.substring(0, 3) + '***'
          : 'MISSING',
      ...
      if (!env.TRAVELTEK_FTP_HOST || !env.TRAVELTEK_FTP_USER || !env.TRAVELTEK_FTP_PASSWORD) {
        throw new Error(
          'Missing FTP credentials: TRAVELTEK_FTP_HOST, TRAVELTEK_FTP_USER, or TRAVELTEK_FTP_PASSWORD not configured'
        );
      }
      ...
      if (this.circuitBreakerState.failureCount >= this.CIRCUIT_FAILURE_THRESHOLD) {
        ...
        logger.error(
          `🚨 Circuit breaker OPENED after ${this.circuitBreakerState.failureCount} FTP connection failures`
        );
      }
    ...
    return {
      ...
      circuitBreakerState: {
        ...
        lastFailureTime: this.circuitBreakerState.lastFailureTime
          ? new Date(this.circuitBreakerState.lastFailureTime).toISOString()
          : null,
      },
      chunkSize: this.CHUNK_SIZE,
    };
  ...
  async getCruiseInfoForLine(lineId: number, megaBatchSize?: number): Promise<CruiseInfo[]> {
    ...
    try {
      ...
      if (results.length === limit) {
        logger.warn(
          `📊 Limited cruise list to ${limit} cruises for line ${lineId} to prevent FTP overload`
        );
      }
      ...
      throw new Error(
        `Failed to get cruise info for line ${lineId}: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async updatePricingFromCachedData(cruiseId: string, data: any): Promise<void> {
    ...
    try {
      // CRITICAL FIX: Capture pricing snapshot before any updates
      logger.info(`📸 Capturing pricing snapshot for cruise ${cruiseId} before update`);
      const { priceHistoryService } = await import('./price-history.service');
      const batchId = await priceHistoryService.captureSnapshot(
        cruiseId,
        'webhook_pricing_update',
        `webhook_${Date.now()}`
      );
      logger.info(`✅ Pricing snapshot captured with batch ID: ${batchId}`);
