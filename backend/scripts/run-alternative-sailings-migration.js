...
async function runMigration() {
  console.log('Running alternative_sailings schema fix migration...\n');
  ...
  try {
    ...
    const newColumns = await db.execute(sql`
      SELECT column_name, data_type
      FROM information_schema.columns
      WHERE table_name = 'alternative_sailings'
      ORDER BY ordinal_position;
    ...
    try {
      await db.execute(sql`
        DROP TABLE IF EXISTS alternative_sailings;
        ALTER TABLE IF EXISTS alternative_sailings_backup RENAME TO alternative_sailings;
      `);
