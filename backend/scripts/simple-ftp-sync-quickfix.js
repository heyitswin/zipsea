...
/**
 * Simple Initial FTP Sync Script - Quick Fix
 ...
 * - Quick fix: converts problematic string values to NULL for integer fields
 * Date: 2025-01-14
 */
...
/**
 * Safe integer conversion - converts strings like "system" to null
 */
...
async function upsertCruise(client, data) {
  ...
  const marketId = safeIntegerConvert(data.marketid);  // Convert "system" to null
  const ownerId = safeIntegerConvert(data.ownerid);    // Convert "system" to null
  ...
  `,
    [
      ...
      marketId, // Now safely converted to null if "system"
      ownerId,  // Now safely converted to null if "system"
      data.nofly === 'Y',
      data.departuk === 'Y',
      data.showcruise !== 'N',
 ...
 */
async function main() {
  console.log('🚀 Zipsea Simple Initial FTP Sync - Quick Fix');
  console.log('==============================================');
  ...
  console.log('🔧 Quick fix: converts "system" strings to NULL for integer fields');
  console.log('');
