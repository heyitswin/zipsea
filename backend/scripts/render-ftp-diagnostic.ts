...
/**
 * RENDER FTP CREDENTIAL DIAGNOSTIC SCRIPT
 * 
 ...
 *   "render-ftp-diagnostic": "tsx scripts/render-ftp-diagnostic.ts"
 * 
 *   Then run via Render shell or manual deployment:
 *   npm run render-ftp-diagnostic
 */
...
interface FtpDiagnosticResult {
  ...
  renderInfo: {
    ...
    renderCommitSha?: string;
  };
}

async function testFtpConnection(host: string, user: string, password: string): Promise<{ success: boolean; error?: string; duration: number }> {
...
async function runRenderFtpDiagnostic(): Promise<FtpDiagnosticResult> {
  logger.info('🚀 RENDER FTP CREDENTIAL DIAGNOSTIC');
  logger.info('=====================================');
  
  const result: FtpDiagnosticResult = {
  ...
  if (ftpRelatedKeys.length > 0) {
    logger.info('FTP-related keys found:', ftpRelatedKeys);
  }
  ...
  // Compare with processed values
  logger.info('\n⚙️ PROCESSED VS RAW COMPARISON:');
  logger.info(`env.TRAVELTEK_FTP_HOST === process.env.TRAVELTEK_FTP_HOST: ${env.TRAVELTEK_FTP_HOST === process.env.TRAVELTEK_FTP_HOST}`);
  logger.info(`env.TRAVELTEK_FTP_USER === process.env.TRAVELTEK_FTP_USER: ${env.TRAVELTEK_FTP_USER === process.env.TRAVELTEK_FTP_USER}`);
  logger.info(`env.TRAVELTEK_FTP_PASSWORD === process.env.TRAVELTEK_FTP_PASSWORD: ${env.TRAVELTEK_FTP_PASSWORD === process.env.TRAVELTEK_FTP_PASSWORD}`);
...
export { runRenderFtpDiagnostic, FtpDiagnosticResult };