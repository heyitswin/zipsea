...
async function runMigration() {
  ...
  try {
    ...
    const result = await client.query(
      "SELECT name, cancellation_policy_url FROM cruise_lines WHERE cancellation_policy_url IS NOT NULL ORDER BY name"
    );
    console.log(`\nUpdated ${result.rows.length} cruise lines with cancellation policy URLs:`);
