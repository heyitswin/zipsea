...
/**
 * Sync script using Drizzle ORM with proper imports
 */
...
console.log('🚢 Traveltek Complete Data Sync (Fixed Drizzle Version)');
console.log('========================================================\n');

// Verify imports
console.log('📦 Verifying table imports...');
const requiredTables = ['cruises', 'cruiseLines', 'ships', 'ports', 'regions', 'cheapestPricing', 'priceHistory', 'itineraries', 'cabinCategories', 'pricing'];
for (const table of requiredTables) {
  if (!schema[table]) {
    console.error(`❌ Missing table: ${table}`);
    console.log('Available tables:', Object.keys(schema).filter(k => typeof schema[k] === 'object' && schema[k]._ && schema[k]._.name));
  }
}
...
// Progress tracking
const PROGRESS_FILE = '.sync-drizzle-fixed.json';
let progress = {};
...
async function processDependencies(data) {
  ...
  // Upsert cruise line
  try {
    await db.insert(schema.cruiseLines).values({
      id: lineId,
  ...
  try {
    ...
    await db.insert(schema.ships).values(shipData).onConflictDoUpdate({
...
async function processCruiseData(data, filePath) {
  ...
  try {
    await db.insert(schema.cruises)
      .values(cruiseData)
      .onConflictDoUpdate({
...
async function processCabins(shipId, cabins) {
  ...
  for (const [cabinCode, cabin] of Object.entries(cabins)) {
    ...
    try {
      await db.insert(schema.cabinCategories).values({
        shipId: shipIdNum,
        code: cabinCode,
...
async function processDetailedPricing(cruiseId, prices) {
  ...
  try {
    ...
    await db.insert(schema.cheapestPricing).values({
      cruiseId: cruiseId,
