 ...
 * - Fixed field type conversion for marketid/ownerid
 * Date: 2025-01-14
 */
...
/**
 * Convert field value to appropriate type for database
 */
function convertFieldValue(value, expectedType) {
  ...
  switch (expectedType) {
    case 'integer':
      // If it's a string like "system", convert to null or a default value
...
async function upsertCruise(client, data) {
  ...
  const marketId = convertFieldValue(data.marketid, 'string');
  const ownerId = convertFieldValue(data.ownerid, 'string');
  ...
  `,
    [
      ...
      marketId, // Converted field
      ownerId,  // Converted field
      convertFieldValue(data.nofly, 'boolean'),
      convertFieldValue(data.departuk, 'boolean'),
      data.showcruise !== 'N',
 ...
 */
async function main() {
  console.log('🚀 Zipsea Simple Initial FTP Sync - Final');
  console.log('==========================================');
  ...
  console.log('🔧 Fixed SQL parameter count and field type conversion');
  console.log('');
