...
/**
 * Optimized Initial FTP Sync Script
 * - Maintains persistent FTP connections
 * - Processes from 2025/09 onwards to current month
 * - Smart resume capability with checkpoints
 * - Detailed progress indicators
 * - Downloads to memory instead of disk
 * Date: 2025-09-04
 */
  ...
  try {
    ...
    const keepAlive = setInterval(async () => {
      try {
        ...
        console.log(`⚠️ Keep-alive failed for connection ${index}, will reconnect`);
      }
...
function displayProgress() {
  ...
  console.clear();
  console.log('🚀 Zipsea Initial FTP Sync - Optimized');
  console.log('======================================\n');
...
/**
 * Process a single cruise data file (simplified version shown earlier)
 * [Using the same implementation as before but with stats tracking]
 */
...
// [Include the helper functions from original script: ensureCruiseLineExists, ensureShipExists, etc.]
 ...
 */
async function main() {
  console.log('🚀 Zipsea Initial FTP Sync - Optimized Version');
  console.log('==============================================');
...
// Helper functions (same as before but with stats tracking)
...
async function syncPricing(client, cruiseId, data) {
  ...
  // Full pricing sync can be done separately
