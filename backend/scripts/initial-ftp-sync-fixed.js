...
/**
 * Fixed Initial FTP Sync Script
 * - Processes from 2025/09 to 2028/12 (or last available)
 * - Fixed FTP keep-alive to avoid concurrent operations
...
const CONFIG = {
  ...
  MEMORY_DOWNLOAD_LIMIT: 10 * 1024 * 1024, // 10MB - files larger than this use temp file
...
async function createFtpConnection(index) {
  ...
  try {
    ...
    // FIXED: Check if connection is busy before sending NOOP
    const keepAlive = setInterval(async () => {
      ...
      if (connection && !connection.busy) {
        try {
          ...
          console.log(`⚠️ Keep-alive failed for connection ${index}, will reconnect when needed`);
        }
      }
 ...
 * FIXED: Process from 2025/09 to 2028/12 instead of just current month
 */
function getMonthsToProcess() {
  ...
  // Process from 2025/09 up to 2028/12 (or whatever is available)
  ...
  console.log(`📅 Will process months from ${CONFIG.START_YEAR}/${String(CONFIG.START_MONTH).padStart(2, '0')} to ${CONFIG.END_YEAR}/${String(CONFIG.END_MONTH).padStart(2, '0')}`);
  return months;
}
...
async function listCruiseFiles(year, month) {
  ...
  try {
    ...
    const yearExists = await connection.client.list('/').then(list =>
    ...
    const monthExists = await connection.client.list(`/${year}`).then(list =>
    ...
    console.error(`   ❌ Error listing files for ${year}/${String(month).padStart(2, '0')}:`, error.message);
    return files;
  } finally {
...
/**
 * Download file to memory or temp file based on size
 */
async function downloadFile(filePath, fileSize, retries = 0) {
  const connection = await getFtpConnection();

  try {
    let data;

    if (fileSize && fileSize < CONFIG.MEMORY_DOWNLOAD_LIMIT) {
      ...
      // Use temp file for large files
      const tempFile = path.join(require('os').tmpdir(), `cruise-${Date.now()}.json`);
      const stream = require('fs').createWriteStream(tempFile);
      await connection.client.downloadTo(stream, filePath);
    ...
    if (retries < CONFIG.MAX_RETRIES) {
      ...
      return downloadFile(filePath, fileSize, retries + 1);
    }
    throw error;
  } finally {
    releaseFtpConnection(connection);
...
function displayProgress() {
  ...
  console.clear();
  console.log('🚀 Zipsea Initial FTP Sync - Fixed Version');
  console.log('==========================================\n');
...
// Helper functions (include all the helper functions from original script)
...
async function upsertCruise(client, data) {
  ...
  if (existing.rows.length === 0) {
    // Insert new cruise (with all fields)
    await client.query(
      `INSERT INTO cruises (
...
async function syncCheapestPricing(client, cruiseId, data) {
  ...
  if (cheapest.outside || data.cheapestoutside) prices.push(cheapest.outside || data.cheapestoutside);
  if (cheapest.balcony || data.cheapestbalcony) prices.push(cheapest.balcony || data.cheapestbalcony);
...
async function main() {
  console.log('🚀 Zipsea Initial FTP Sync - Fixed Version');
  console.log('==========================================');
  console.log(`📅 Processing from ${CONFIG.START_YEAR}/${String(CONFIG.START_MONTH).padStart(2, '0')} to ${CONFIG.END_YEAR}/${String(CONFIG.END_MONTH).padStart(2, '0')}`);
