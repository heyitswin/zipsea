...
/**
 * COMPREHENSIVE PRODUCTION DATA PIPELINE TEST
 * ============================================
 * THIS IS THE SOURCE OF TRUTH FOR DATA INTEGRITY
 *
 * Tests EVERYTHING in the production pipeline:
 * - Webhook events and processing status
 * - FTP data structure validation
 * - Database integrity and constraints
 * - Price extraction correctness
 * - API response accuracy
 * - Data consistency across all layers
 *
 * WARNING: This is a HEAVY test that queries large amounts of data
 * Designed to give you COMPLETE confidence in your data
 *
 * Run: DATABASE_URL=<production_url> node scripts/test-production-comprehensive.js
 */
...
async function testWebhookPipeline() {
  ...
  try {
    ...
    // 1.2 Check webhook processing statistics
    ...
    // 1.3 Check webhook event types distribution
    ...
    // 1.4 Check for webhook failures patterns
    const failurePatterns = await db.execute(sql`
      ...
      GROUP BY line_id, DATE(created_at), metadata->>'error'
      HAVING COUNT(*) > 5
...
async function testPriceExtraction() {
  ...
  try {
    // 3.1 Get sample of cruises with raw_data to validate extraction
    ...
    // 3.2 Check price consistency between tables
    const priceConsistency = await db.execute(sql`
      SELECT
        COUNT(*) as total,
        COUNT(CASE
          WHEN ABS(COALESCE(c.interior_price, 0) - COALESCE(cp.interior::numeric, 0)) > 0.01
          THEN 1
        END) as interior_mismatch,
        COUNT(CASE
          WHEN ABS(COALESCE(c.oceanview_price, 0) - COALESCE(cp.oceanview::numeric, 0)) > 0.01
          THEN 1
        END) as oceanview_mismatch,
        COUNT(CASE
          WHEN ABS(COALESCE(c.balcony_price, 0) - COALESCE(cp.balcony::numeric, 0)) > 0.01
          THEN 1
        END) as balcony_mismatch,
        COUNT(CASE
          WHEN ABS(COALESCE(c.suite_price, 0) - COALESCE(cp.suite::numeric, 0)) > 0.01
          THEN 1
...
async function testDataFreshness() {
  ...
  try {
    ...
    for (const line of linesFreshness) {
      ...
      test.details.linesFreshness.push(lineData);
      ...
      results.cruiseLines[line.id].freshness = lineData;
    ...
    for (const line of priceCoverage) {
      ...
      test.details.priceCoverage.lines.push(lineData);
      ...
      results.cruiseLines[line.id].priceCoverage = lineData;
...
async function testComprehensiveValidation() {
  ...
  try {
    ...
    // 6.5 Check webhook to cruise update correlation
    const updateCorrelation = await db.execute(sql`
      WITH recent_webhooks AS (
        ...
        GROUP BY line_id, DATE(created_at)
      ),
      recent_updates AS (
        SELECT
          cruise_line_id,
...
async function runComprehensiveTest() {
  console.log('\n' + '='.repeat(80));
  log('COMPREHENSIVE PRODUCTION DATA PIPELINE TEST', 'title');
  ...
  if (!process.env.DATABASE_URL) {
    ...
    log('Run with: DATABASE_URL=<production_url> node scripts/test-production-comprehensive.js', 'error');
    process.exit(1);
  }
