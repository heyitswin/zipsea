...
/**
 * Complete data sync script with UPSERT functionality
 * - Uses INSERT ... ON CONFLICT DO UPDATE for all operations
 * - Ensures data is never skipped, always updated
...
console.log('🚢 Complete Traveltek Data Sync (with UPSERT)');
console.log('==============================================\n');
...
async function processCompleteCruise(client, filePath) {
  try {
    ...
    const wasInserted = await upsertCruiseData(data, filePath);
...
/**
 * UPSERT cruise data - returns true if inserted, false if updated
 */
async function upsertCruiseData(data, filePath) {
  ...
  // Check if it was an insert (xmax = 0) or update
  return result.rows && result.rows[0] && result.rows[0].inserted;
}
...
async function processDirectory(client, dirPath) {
  try {
    ...
    for (const file of jsonFiles) {
      ...
      // Always process with FORCE_UPDATE
...
// Run sync
console.log('Starting complete data sync with UPSERT...');
console.log('This version will UPDATE existing cruises with any new data\n');
