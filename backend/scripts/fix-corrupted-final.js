/**
 * FINAL WORKING VERSION - Direct check for numeric keys
 * Based on diagnostic showing raw_data HAS numeric keys directly
 */
...
function isCharacterArray(rawData) {
  // Simple direct check - does it have numeric string keys?
  // Don't check typeof, just check for the keys
  if (rawData && rawData['0'] !== undefined && rawData['1'] !== undefined && rawData['2'] !== undefined) {
    // Check if the values are single characters
    if (rawData['0'].length === 1 && rawData['1'].length === 1) {
      return true;
    }
  }
  return false;
...
function reconstructJsonFromCharArray(rawData) {
  if (!isCharacterArray(rawData)) {
    // If it's not a character array but is a string, try to parse it
...
async function main() {
  ...
  try {
    ...
    // Test detection on known corrupted cruise
    console.log('\nTesting detection on cruise 2144014...');
    ...
    if (testCruise.length > 0) {
      const rd = testCruise[0].raw_data;
      console.log(`  Type of raw_data: ${typeof rd}`);
      console.log(`  Has key '0': ${rd['0'] !== undefined}`);
      console.log(`  Has key '1': ${rd['1'] !== undefined}`);
      if (rd['0']) {
        console.log(`  Value at '0': "${rd['0']}" (length ${rd['0'].length})`);
      }
      console.log(`  isCharacterArray: ${isCharacterArray(rd)}`);
    }
    console.log('');
    ...
    while (hasMore) {
      ...
      for (const cruise of batch) {
        // Debug first cruise in first batch
        if (batchCount === 0 && batchCorrupted === 0 && cruise.raw_data['0'] !== undefined) {
          console.log(`  DEBUG first cruise with numeric keys:`);
          console.log(`    ID: ${cruise.id}`);
          console.log(`    Has '0': ${cruise.raw_data['0'] !== undefined}`);
          console.log(`    Value at '0': "${cruise.raw_data['0']}"`);
          console.log(`    Detection result: ${isCharacterArray(cruise.raw_data)}`);
        }
      ...
      if (batchFixed > 0) {
        ...
        console.log(`  No corruption found in this batch`);
      }
    ...
    if (!DRY_RUN) {
      ...
      console.log('\n📌 Corruption detected! To fix the data, run:');
      console.log('   node scripts/fix-corrupted-final.js --execute');
    }
