...
export default function OnboardCreditCalculator() {
  ...
  const autoAllocateCredit = (items: SpendingItem[], pricing: CruiseLinePricing) => {
  ...
  const handleQuantityChange = (id: string, newQuantity: number) => {
    ...
    const newTotal = updatedSpending.reduce((total, item) =>
      ...
      <main style={{ backgroundColor: '#E9B4EB' }} className="py-[40px] md:py-[80px]">
        <div className="max-w-6xl mx-auto px-8">
          ...
          <div className="bg-white rounded-lg p-6 md:p-8 mb-8 shadow-sm">
            <p className="font-geograph text-[16px] md:text-[18px]" style={{ color: '#0E1B4D' }}>
              Use this calculator to plan how you'll spend your onboard credit during your cruise.
              Enter your total credit amount, select your cruise line, and customize how you want
              to allocate your spending across various onboard experiences.
            </p>
            <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="font-geograph text-[14px] md:text-[16px] text-gray-700">
                <strong>Disclaimer:</strong> Prices shown are estimates based on average cruise line pricing
                and may vary depending on your specific ship, sailing date, and itinerary.
                Actual onboard prices may differ. Gratuities shown are per day, per person.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-lg p-6 shadow-sm sticky top-8">
                <h2 className="font-geograph font-bold text-[24px] mb-6" style={{ color: '#0E1B4D' }}>
                  Your Cruise Details
                </h2>

                {/* OBC Amount */}
                <div className="mb-6">
                  <label className="font-geograph font-medium text-[16px] mb-2 block" style={{ color: '#0E1B4D' }}>
                    Onboard Credit Amount ($)
                  </label>
                  <input
                    type="number"
                    value={obcAmount}
                    onChange={(e) => setObcAmount(Math.max(0, parseInt(e.target.value) || 0))}
                ...
                <div className="mb-6">
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg font-geograph text-[18px] bg-white"
                    style={{ color: '#0E1B4D' }}
                  >
                ...
                <div className="mb-6">
                  ...
                  <input
                    type="number"
                    value={cruiseDays}
                    onChange={(e) => setCruiseDays(Math.max(1, parseInt(e.target.value) || 1))}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg font-geograph text-[18px]"
                    style={{ color: '#0E1B4D' }}
                    min="1"
                    max="30"
                ...
                <div className="pt-6 border-t border-gray-200">
                  <div className="flex justify-between mb-3">
                    <span className="font-geograph text-[16px]" style={{ color: '#0E1B4D' }}>
                      Total Credit:
                    </span>
                    <span className="font-geograph font-bold text-[18px]" style={{ color: '#0E1B4D' }}>
                      ${obcAmount.toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between mb-3">
                  ...
                  >
                    <span className="font-geograph font-medium text-[16px]" style={{ color: '#0E1B4D' }}>
                      {remainingCredit >= 0 ? 'Remaining Credit:' : 'Over Budget:'}
                    </span>
                    <span
                  ...
                  {remainingCredit > 0 && (
                    <p className="mt-3 text-[14px] font-geograph text-gray-600">
                      Tip: Remaining credit can be used for miscellaneous purchases,
                      additional gratuities, or saved for onboard shopping!
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-lg p-6 shadow-sm">
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                        ...
                        <div className="md:col-span-1">
                          <h3 className="font-geograph font-medium text-[18px]" style={{ color: '#0E1B4D' }}>
                            {item.name}
                          </h3>
                        ...
                        <div className="md:col-span-1 text-right">
                          <p className="font-geograph font-bold text-[20px]" style={{ color: '#0E1B4D' }}>
                      ...
                      {item.id === 'gratuities' && item.quantity > 0 && (
                        <p className="text-[13px] font-geograph text-gray-500 mt-2">
                          Covers automatic daily gratuities for your stateroom attendant and dining staff
                        </p>
                      )}
                      {item.id === 'beverage' && item.quantity > 0 && cruiseLine === 'Disney Cruise Line' && (
                        <p className="text-[13px] font-geograph text-amber-600 mt-2">
                          Note: Disney Cruise Line doesn't offer beverage packages. Consider individual drinks instead.
                        </p>
                      )}
                    </div>
                  ))}
                ...
                <div className="mt-8 p-6 bg-blue-50 rounded-lg">
                  <h3 className="font-geograph font-bold text-[18px] mb-3" style={{ color: '#0E1B4D' }}>
                    💡 Smart Spending Tips
                  </h3>
                  <ul className="font-geograph text-[14px] space-y-2" style={{ color: '#0E1B4D' }}>
                    <li>• Book specialty dining and spa services early for best availability</li>
                    <li>• Consider if you'll actually use a full beverage package vs. buying drinks à la carte</li>
                    <li>• Shore excursions booked through the cruise line ensure the ship waits if you're late</li>
                    <li>• WiFi packages are usually cheaper when purchased for the full voyage</li>
                    <li>• Photo packages capture memories but check if individual photos might be more economical</li>
                  </ul>
                </div>
              </div>
            </div>
