...
export async function sendSlackQuoteNotification(quoteData: QuoteData) {
  ...
  try {
    ...
    const passengerInfo = `${quoteData.passengers.adults} adult${quoteData.passengers.adults !== 1 ? 's' : ''}${
      quoteData.passengers.children > 0 ? `, ${quoteData.passengers.children} child${quoteData.passengers.children !== 1 ? 'ren' : ''}` : ''
    }`;

    // Format discount qualifiers
    const activeDiscounts = [];
    if (quoteData.discounts.payInFull) activeDiscounts.push('Pay in full/non-refundable');
    ...
    if (quoteData.discounts.stateOfResidence) activeDiscounts.push(`Resident of ${quoteData.discounts.stateOfResidence}`);
    if (quoteData.discounts.loyaltyNumber) activeDiscounts.push(`Loyalty: ${quoteData.discounts.loyaltyNumber}`);
